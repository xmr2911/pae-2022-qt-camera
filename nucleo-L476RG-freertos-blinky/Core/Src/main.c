...
int main(void)
{
  ...
  Write_Flash(PREVIOUS_STATE_ADDR, &test, 1);
  Write_Flash(DEPLOYMENT_STATE_ADDR, &provaa,1);
  Write_Flash(DEPLOYMENTRF_STATE_ADDR, &test, 1);
  Read_Flash(CURRENT_STATE_ADDR, &read,1);
  Write_Flash(NOMINAL_ADDR, &lol,1);
  /* USER CODE END 2 */
...
/* USER CODE BEGIN Header_StartOBC */
/**
  * @brief  Function implementing the OBC thread.
  * @param  argument: Not used
  * @retval None
  */
/* USER CODE END Header_StartOBC */
void StartOBC(void *argument)
{
  /* USER CODE BEGIN 5 */
  /* Infinite loop */
  for(;;)
  {
    ...
    osDelay(2000);
    osThreadFlagsSet(COMMSHandle, 2);
  }
...
void StartCOMMS(void *argument)
{
  ...
  for(;;)
  {
    ...
    uint8_t comms = 9;
    uint32_t receive;
    receive = osThreadFlagsWait(2, osFlagsWaitAny, osWaitForever);
    Write_Flash(CURRENT_STATE_ADDR, &comms, sizeof(comms));
  }
  /* USER CODE END StartCOMMS */
}

/* USER CODE BEGIN Header_StartADCS */
/**
* @brief Function implementing the ADCS thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_StartADCS */
void StartADCS(void *argument)
{
  /* USER CODE BEGIN StartADCS */
  /* Infinite loop */
  for(;;)
  {
    osDelay(1);
  }
  /* USER CODE END StartADCS */
}
